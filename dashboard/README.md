...
시스템 페이지의 `Hikvision 카메라 3대 설정`에서 이름, JPEG 주소, 사용자명과 비밀번호를 저장하고 바로 촬영 시험을 할 수 있습니다. 새 카메라는 모두 기본 IP `192.168.1.64`를 사용할 수 있으므로 한 대씩 활성화한 뒤 `192.168.1.60`, `.61`, `.62`처럼 서로 다른 고정 IP를 먼저 지정해야 합니다. 대시보드는 00·06·12·18시 정기 촬영과 수동 `지금 촬영` 결과를 카메라별로 표시합니다.
