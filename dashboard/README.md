...
- Pico 2 W USB 센서 수집: RS485 SHTC3 온습도, KCD-HP100 CO₂, PE350 EC/pH
...
- RS485 CO₂ 센서를 의도적으로 제거한 설치에서만 `SMARTFARM_CO2_REQUIRED=0`으로
  두면 SHTC3·PE350 실측값만으로 제어 근거를 판정합니다.
...
5. Pico의 `config.py`에 현재 RS485 국번·속도 설정이 저장되어 있는지 확인합니다.
...
`SMARTFARM_SUPPLY_CONTINUOUS_ENABLED=1`이면 GP18의 양액 공급 펌프가 24시간
연속 운전합니다. Pico는 86,400초의 장치 측 타이머를 사용하며, 서버는 USB 재연결이나
Pico 재시작 뒤 15초 안에 다시 ON 명령을 전송하고 ACK·실패를 감사 기록에 남깁니다.
...
A+B 양액펌프 보정, pH가 6.5 초과이면 산성액 보정, pH가 관리 하한보다 낮으면 원수
10초 1회 보정의 텔레그램 승인 요청을 보냅니다. 원수 승인 후에는 교반·PE350 재측정을
먼저 하고, 실제 EC가 관리 하한보다 낮을 때에만 A+B의 별도 텔레그램 승인 요청을 보냅니다.
승인된 A+B/산성액 보정 세션은 설정된 시간의 주입 → 30초 교반 → PE350 재측정을 목표 범위에
도달할 때까지 반복하며, EC 상한 초과 또는 pH 하한 아래에서 즉시 끝납니다.
즉, 최초 텔레그램 승인 뒤에는 목표 범위까지 자동 반복하며, 새 알림은 같은 항목에 대해 기본
15분 간격으로 제한됩니다.
...
`public_dashboard/`는 다른 Wi-Fi에서도 열 수 있는 별도 Vercel 프로젝트입니다.
학교 서버가 HMAC으로 서명한 센서 요약과 최신 카메라 사진만 60초마다 전송하며,
그 프로젝트에는 Pico USB·MQTT 명령·릴레이 제어·설정 API가 없습니다. Vercel Blob에는
공개 URL의 사진이 저장되므로 학교 내부용 사진이나 개인정보가 프레임 안에 들어가지
않도록 촬영 구도를 먼저 확인해야 합니다.

배포와 환경변수 설정은 [public_dashboard/README.md](../public_dashboard/README.md)를 따릅니다.
공개 동기화가 구성되기 전까지는 `SMARTFARM_PUBLIC_SYNC_ENABLED=0`을 유지합니다.
